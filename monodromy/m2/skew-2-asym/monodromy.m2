...
-- Asymmetric
yDepths = yDepths * inverse diagonalMatrix matrix toList apply(1..npoints, i -> yScale)
zDepths = zDepths * inverse diagonalMatrix matrix toList apply(1..npoints, i -> zScale)

depths = xDepths_{1..(numcols xDepths - 1)} | yDepths_{0..0} | yDepths_{2..numcols yDepths - 1} | zDepths_{0..1} | zDepths_{3..numcols zDepths - 1}
pointMatrix = depths | matrix yScale | matrix zScale
pointMatrix = if contains(symbol s, params) then matrix ps*pfy | pointMatrix else pointMatrix
...
unknownMatrix = gateMatrix{vars(
	...
	-- 1st view relative depths
	{a_2, a_3, a_4, a_5, b_1, b_3, b_4, b_5, c_1, c_2, c_4, c_5, z_2, z_3}
	)};
...
eqsXY = apply(sort(subsets(npoints,2)), S->((i,j) := (first sort S, last sort S); compress(nsq(a_(i+1) * xPointsSymbolic_{i} - a_(j+1) * xPointsSymbolic_{j}) - nsq(b_(i+1) * z_2 * yPointsSymbolic_{i} - b_(j+1) * z_2 * yPointsSymbolic_{j}))));
eqsXZ = apply(sort(subsets(npoints,2)), S->((i,j) := (first sort S, last sort S); compress(nsq(a_(i+1) * xPointsSymbolic_{i} - a_(j+1) * xPointsSymbolic_{j}) - nsq(c_(i+1) * z_3 * zPointsSymbolic_{i} - c_(j+1) * z_3 * zPointsSymbolic_{j}))));
...
func = (i,R) -> (
    ...
    (V, npaths) = monodromySolve(GS, z0, {x0}, Verbose=>true, NumberOfNodes=>2, NumberOfEdges=>3);
