...
pointMatrix = if contains(symbol fyi, params) then matrix pfy^2 | pointMatrix else pointMatrix
pointMatrix = if contains(symbol fxi, params) then matrix pfx^2 | pointMatrix else pointMatrix
...
diac = compress(matrix {
    {efy,                -es,                                    ecy*es - ecx*efy},
    {-es,                es^2/efy + efx,                         ecx*es - ecy*efx - (ecy*es^2)/efy},
    {ecy*es - ecx*efy,   ecx*es - ecy*efx - (ecy*es^2)/efy,      efy*ecx^2 - 2*ecx*ecy*es + (ecy^2*es^2)/efy + efx*ecy^2 + efx*efy}
} * matrix {{one/(efx*efy), 0, 0}, {0, one/(efx*efy), 0}, {0, 0, one/(efx*efy)}});
...
func = (i,R) -> (
    ...
    (V, npaths) = monodromySolve(GS, z0, {x0}, Verbose=>true, NumberOfNodes=>2, NumberOfEdges=>3, BatchSize=>MonodromyBatchSize);
